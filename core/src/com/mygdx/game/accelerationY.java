...
public class accelerationY implements Function{
    ...
    public double f(double x, double y,double vx,double vy) throws FileNotFoundException {
        ...
        */
        // Sophisticated
        return -m*g*px.getY(x,y,vx,vy)/(1 + px.getX(x,y,vx,vy)*px.getX(x,y,vx,vy) + px.getY(x,y,vx,vy)*px.getY(x,y,vx,vy)) - Field.frictionKinetic*m*g/Math.sqrt(1 + px.getX(x,y,vx,vy)*px.getX(x,y,vx,vy) + px.getY(x,y,vx,vy)*px.getY(x,y,vx,vy))*(vy/Math.sqrt(vx*vx + vy*vy + Math.pow(px.getX(x,y,vx,vy)*vx + px.getY(x,y,vx,vy)*vy, 2)))/m;
    }
}
