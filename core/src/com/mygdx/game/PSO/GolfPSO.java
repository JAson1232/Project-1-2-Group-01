...
//  }
    ...
    public void initializeBestPrtclFitness(Vector hole){
        for(int i=0;i<SWARM_SIZE;i++){
            swarm[i].setBestPrtclFitness(swarm[i].getFitness(-3,0, hole));
        }
    }


    //initialise gBest with best fit particle
    public void initializeGBest(Vector hole){
        swarmBestFitness = swarm[0].getFitness(-3,0, hole);
        swarmBestPosition = swarm[0].getLocation();
        //System.out.println("initial fitness value for ball at (-3,0)"+swarmBestFitness);
        //System.out.println("initial swarm Best Po, sition"+ swarmBestPosition);
    ...
    public void updateIndBest(int i,Vector s, Vector hole){
        if (swarm[i].getBestPrtcleFitness() > swarm[i].getFitness(s.getX(),s.getY(),hole)) {
            swarm[i].setBestPrtclFitness(swarm[i].getFitness(s.getX(),s.getY(),hole));
            swarm[i].setPrtclBestPosition(swarm[i].getLocation());
    ...
    public void updateSwarmBest(int i,Vector st, Vector hole){
        if(swarmBestFitness > swarm[i].getFitness(st.getX(),st.getY(), hole)){
            swarmBestFitness = swarm[i].getFitness(st.getX(),st.getY(), hole);
            swarmBestPosition = swarm[i].getLocation();
...
//    }
     ...
     */
    public Vector runPSO(Vector state, Vector hole) throws FileNotFoundException{
...
//        Ball ball = new Ball(state);
        ...
        initializeBestPrtclFitness(hole);
        initializeParticleBestPosition();
        initializeGBest(hole);
...
//                state = ball.getState();
                ...
                updateIndBest(i,state,hole);
                updateSwarmBest(i,state, hole);
                updatePosition(i);
                //System.out.println("------------");
                //System.out.println(swarmBestFitness);
            }
        ...
        state.setX(stateCopy.getX());
        state.setY(stateCopy.getY());
