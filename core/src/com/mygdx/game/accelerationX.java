...
//        && px.getX(x,y,vx,vy) < Field.frictionStatic && px.getY(x,y,vx,vy) < Field.frictionStatic) {
        if ((Math.sqrt(((vx) * (vx)) + ((vy) * (vy)))) < Experiments.h * 5) {
            ...
            // Sophisticated
            //return -m * g * px.getX(x, y, vx, vy) / (1 + px.getX(x, y, vx, vy) * px.getX(x, y, vx, vy) + px.getY(x, y, vx, vy) * px.getY(x, y, vx, vy)) - Field.frictionKinetic * m * g / Math.sqrt(1 + px.getX(x, y, vx, vy) * px.getX(x, y, vx, vy) + px.getY(x, y, vx, vy) * px.getY(x, y, vx, vy)) * (vx / Math.sqrt(vx * vx + vy * vy + Math.pow(px.getX(x, y, vx, vy) * vx + px.getY(x, y, vx, vy) * vy, 2)));
        }
        // Original
